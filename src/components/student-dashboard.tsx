...
export function StudentDashboard({
  ...
  return (
    <main className="paper-grid min-h-screen min-w-0 max-w-full overflow-x-clip px-4 py-6 sm:px-6 sm:py-10">
      <div className="mx-auto w-full min-w-0 max-w-5xl space-y-6">
        <header className="surface-glass accent-edge sticky top-3 z-30 relative overflow-visible rounded-2xl border border-white/10 p-4 shadow-[0_16px_40px_rgba(0,0,0,.2)] backdrop-blur-xl sm:p-7">
          ...
          <div className="relative flex min-w-0 flex-wrap items-start justify-between gap-6">
            <div className="min-w-0 max-w-full">
              <p className="accent-kicker">Student profile</p>
              <h1 className="break-words font-display text-3xl font-semibold sm:text-4xl">Welcome, {profile.name}</h1>
              <p className="mt-2 max-w-full break-words text-sm text-text-secondary">{student.admission_no ? `Admission No. ${student.admission_no}` : profile.email}</p>
            </div>
            <div className="flex min-w-0 max-w-full items-start gap-2">
              <div className="w-full min-w-0 rounded-xl border border-[var(--border)] bg-surface-0/80 px-4 py-3 text-left sm:w-auto sm:min-w-36 sm:text-right">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-text-secondary">Class</p>
                <p className="mt-1 break-words font-semibold">{placementLabel}</p>
        ...
        {panel && panel !== "options" && <section ref={panelRef} className="scroll-mt-6 min-w-0 max-w-full overflow-hidden rounded-xl border border-[var(--border)] bg-surface-1 p-4 sm:p-6">
          <div className="flex min-w-0 items-center justify-between gap-3"><h2 className="min-w-0 break-words font-display text-2xl font-semibold">{panel === "profile" ? "Edit profile" : panel === "guardian" ? "Guardian info" : panel === "results" ? "Assessment record" : panel === "settings" ? "Account settings" : "Class details"}</h2><button type="button" onClick={() => setPanel(null)} className="min-h-10 shrink-0 rounded-lg border border-[var(--border)] px-3 text-sm font-semibold">Close</button></div>
