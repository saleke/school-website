-- Step 4: rank within section cohorts when a section is selected.
...
begin
  for subject_row in select id, class_id from public."Subject" loop
    for student_row in select s.id from public."Student" s where s.class_id = subject_row.class_id loop
      select coalesce(sum(sc.raw_score), 0) into average
      ...
      on conflict (student_id, subject_id, term_id) do update set weighted_average = excluded.weighted_average, computed_at = now();
    end loop;
  end loop;
  with ranked as (
    select snapshot.id,
    ...
    where snapshot.term_id = target_term_id and snapshot.subject_id is not null
  )
